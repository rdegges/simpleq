...
@pytest.mark.live
@pytest.mark.asyncio
async def test_live_fifo_receive_supports_request_attempt_id() -> None:
    _load_dotenv(Path(".env"))
    required = [
        "AWS_ACCESS_KEY_ID",
        "AWS_SECRET_ACCESS_KEY",
    ...
    simpleq = _live_simpleq(wait_seconds=0, visibility_timeout=2)
    queue = simpleq.queue(
        f"simpleq-live-attempt-{uuid4().hex[:8]}.fifo",
        fifo=True,
        content_based_deduplication=False,
        wait_seconds=0,
    )
    ...
    try:
        ...
        assert received[0].args == ("attempt-id",)
